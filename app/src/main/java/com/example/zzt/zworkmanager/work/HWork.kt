 ...
 * 工作单元
 */
class HWork : CoroutineWorker {
    val TAG = HWork::class.java.simpleName
    var maxCount = 20
    ...
    suspend fun timeWork(): Result {
        var count = 0;
        setProgress(workDataOf("pro" to "0%"))
        while (true) {
            count++
            if (count >= maxCount) {
                break
            }
//            setProgressAsync(workDataOf("progress Async" to "${count / maxCount}%"))
            setProgress(workDataOf("pro" to "${count}%"))
            Log.v(
                TAG, "work 工作进度 :" + count +
        ...
        setProgress(workDataOf("pro" to "100%"))
